  ...
  SELECT
    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'flavor', w.flavor,
        'quantity', w.quantity,
        'price_per_unit', w.current_price_per_unit,
        'product_image', w.product_image,
        'brand_slug', w.brand_slug,
        'retail_price_per_unit', w.retail_price_per_unit,
        'guaranteed_price_per_unit', w.guaranteed_price_per_unit,
        'super_price_per_unit', w.super_price_per_unit,
        'product_link', w.product_link
      ) ORDER BY w.created_at
    ) AS items,
    COALESCE(SUM(w.quantity * w.current_price_per_unit), 0) AS subtotal,
    COALESCE(SUM(w.quantity * COALESCE(w.retail_price_per_unit, w.current_price_per_unit)), 0) AS full_price
  FROM public.user_orders u
  JOIN public.waiting_list_items w ON w.user_id = u.user_id
  WHERE u.order_type = 'collective'
    AND u.status = 'pending'
    AND u.created_at >= date_trunc('week', now())
  GROUP BY u.id
)
UPDATE public.user_orders u
SET items = rebuilt.items,
    subtotal = rebuilt.subtotal,
    total_amount = rebuilt.subtotal + COALESCE(u.delivery_cost, 0),
    discount_amount = GREATEST(rebuilt.full_price - rebuilt.subtotal, 0),
    updated_at = now()
FROM rebuilt
...
BEGIN
  FOR r IN
    SELECT slug FROM public.brand_overrides
  LOOP
    PERFORM public.refresh_brand_goal(r.slug);
  END LOOP;
END;
$$;