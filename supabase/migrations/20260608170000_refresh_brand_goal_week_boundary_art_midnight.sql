-- Align refresh_brand_goal "this week" boundary to Monday 00:00 ART.
-- Previously date_trunc('week', now()) resolved to Monday 00:00 UTC (= Sunday 21:00 ART),
-- which straddled the weekly reset boundary and counted late-Sunday orders into the wrong week.
-- Now matches the weekly reset cron + booster cycle, all pivoting at Monday 00:00 ART.
...
BEGIN
  ...
  -- Week boundary = Monday 00:00 ART (matches weekly reset cron + booster cycle)
  week_start := date_trunc('week', now() AT TIME ZONE 'America/Argentina/Buenos_Aires') AT TIME ZONE 'America/Argentina/Buenos_Aires';
