-- Report v3: exclude admin devices/accounts; orders by status + cancelled stats;
-- abandoned cart value estimate.
...
declare
  result jsonb;
begin
  if not (public.has_role(auth.uid(), 'admin'::app_role) or coalesce(auth.jwt()->>'role','') = 'service_role') then raise exception 'forbidden'; end if;

  with admin_users as (
    select ur.user_id from user_roles ur where ur.role = 'admin'
  ),
  admin_visitors as (
    select distinct ae.visitor_id from analytics_events ae
    where ae.user_id in (select user_id from admin_users)
  ),
  ev as (
    select * from analytics_events
    where created_at >= p_from and created_at < p_to
      and (path is null or path not like '/admin%')
      and visitor_id not in (select visitor_id from admin_visitors)
  ),
  sess as (
    select session_id,
  ...
  firsts as (
    ...
    where visitor_id not in (select visitor_id from admin_visitors)
    group by visitor_id
  ),
  orders_all as (
  ...
  orders_p as (
    select * from orders_all where status::text <> 'cancelled'
  ),
  order_attr as (
  ...
  signups_p as (
    ...
    where p.created_at >= p_from and p.created_at < p_to
      and p.user_id not in (select user_id from admin_users)
  )
  select jsonb_build_object(
    ...
    'carts', (
      select jsonb_build_object(
        ...
        'abandoned_value', (
          select coalesce(sum(
            (a.props->>'price')::numeric * coalesce(nullif(a.props->>'qty','')::numeric, 1)), 0)
          from ev a
          where a.event in ('Add to Cart', 'Group Joined', 'List Joined')
            and a.props->>'price' ~ '^[0-9]+(\.[0-9]+)?$'
            and not exists (
              select 1 from ev c where c.event = 'Checkout Complete' and c.visitor_id = a.visitor_id)
        )
      )
      ...
      select jsonb_build_object(
        ...
        'by_status', coalesce((
          select jsonb_object_agg(st, cnt) from (
            select status::text as st, count(*) as cnt from orders_all group by status) bs
        ), '{}'::jsonb),
        'cancelled', (
          select jsonb_build_object(
            'count', count(*),
            'amount', coalesce(sum(total_amount), 0))
          from orders_all where status::text = 'cancelled'
        )
      ) from orders_p
    ),
    'referral', (
      select jsonb_build_object(
        'clicks', (select count(*) from referral_clicks
                    where created_at >= p_from and created_at < p_to),
        'referred_signups', (select count(*) from profiles pr
                              where pr.referred_by is not null
                                and pr.created_at >= p_from and pr.created_at < p_to
                                and pr.user_id not in (select user_id from admin_users)),
