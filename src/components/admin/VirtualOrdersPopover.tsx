...
import VirtualSpeedSlider from "./VirtualSpeedSlider";
...
const VirtualOrdersPopover = ({
  ...
  const getSpeedFromProbability = (prob: number | null): number => {
    if (prob === null || !Number.isFinite(prob) || prob <= 0) return DEFAULT_SPEED;

    // prob = DEFAULT_BASE_PROBABILITY * speed
    const calculated = prob / DEFAULT_BASE_PROBABILITY;

    const minSpeed = speedMarks[0];
    const maxSpeed = speedMarks[speedMarks.length - 1];

    // If the stored value is outside our UI's expected range, treat as default.
    // (Prevents legacy/random values from showing as 2.0 by accident.)
    if (calculated < minSpeed * 0.8 || calculated > maxSpeed * 1.2) return DEFAULT_SPEED;
  ...
  // Refresh local state when opening and when props change (e.g., after weekly reset)
  useEffect(() => {
    if (!open) return;
    setVirtualCount(currentVirtualCount);
    setSpeed(getSpeedFromProbability(currentBaseProbability));
    setMode(getCurrentMode());
  }, [open, currentVirtualCount, currentBaseProbability, isManual]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent 
        className="w-80 bg-popover text-popover-foreground p-4 shadow-lg z-50 border rounded-lg" 
        align="end"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="space-y-4">
          <h4 className="font-medium text-sm text-foreground truncate" title={productName}>
            {productName}
          </h4>
          ...
          <div className="space-y-2">
            ...
            <Select value={mode} onValueChange={(value: VirtualMode) => setMode(value)}>
              ...
              <SelectContent className="bg-popover z-50">
                ...
                <SelectItem value="active_24h">Activa 24h</SelectItem>
          ...
          {/* Speed Slider */}
          <VirtualSpeedSlider marks={speedMarks} value={speed} onValueChange={setSpeed} />

          {/* Manual Virtual Orders Input */}
          <div className="space-y-2">
            <Label className="text-sm text-muted-foreground">Ajustar Órdenes Virtuales:</Label>
